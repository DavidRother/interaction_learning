        ...
        for i, agent in enumerate(world.agents):
            if rand_num < self.prob_random:
                ...
                if i > 0:
                    ...
                    distance = np.random.uniform(2.5, 2.5)
                    if random.random() > 0.5:
                        sampled_angle = np.random.uniform(np.deg2rad(180), np.deg2rad(270))
                    else:
                        sampled_angle = np.random.uniform(np.deg2rad(270), np.deg2rad(360))
